...
k_eff_tvp = model.set_variable('_tvp', 'k_eff')  # 新增k_eff tvp
...
lterm = (ppfd - ppfd_ref) ** 2 + 0.0001 * (pwm) ** 2  # 从0.01减到0.0001
mpc.set_objective(mterm=mterm, lterm=lterm)
mpc.set_rterm(pwm=0.0001)  # 从0.01减到0.0001
...
def tvp_fun(t_now):
    ...
    for i in range(N_horizon+1):
        ...
        if step >= sim_steps:
            step = sim_steps - 1
        T_now = T_traj[step]
        k_eff = k * (1 - alpha * (T_now - T_base))
        k_eff = max(k_eff, 0)
        tvp_template['_tvp', i, 'ppfd_ref'] = ppfd_target[step]
        tvp_template['_tvp', i, 'T'] = T_traj[step]
        tvp_template['_tvp', i, 'k_eff'] = k_eff
    return tvp_template
...
def tvp_fun_sim(t_now):
    ...
    tvp_template['T'] = T_traj[step]
    tvp_template['k_eff'] = k_eff
    return tvp_template
...
for t in range(sim_steps):
    ...
    # 温度扰动项
    T_now = T_traj[t]
    x0 = x0 + (T_now - T_base) * beta
    print(f"第{t}步，温度={T_now:.2f}，扰动后ppfd={float(x0[0]):.2f}")
...
plt.legend()
plt.title('MPC closed-loop simulation with temperature drift (re-optimizing target PPFD every 15 min)')
plt.tight_layout()
plt.show()

# 第二张图：温度轨迹
plt.figure(figsize=(10, 6))
plt.plot(T_log, label='Temperature (with noise)', color='tab:red')
plt.ylabel('Temperature (°C)')
plt.xlabel('Time step (min)')
plt.legend()
plt.title('Simulated Temperature Trajectory (with noise)')
plt.tight_layout()
plt.show()
