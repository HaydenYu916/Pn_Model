...
from pymoo.operators.crossover.gpt_openai import GPT_OpenAI_interface
...
class MOEAD_LLM(LoopwiseAlgorithm, GeneticAlgorithm):
    ...
    def __init__(self,
                 ...
                 crossover=GPT_OpenAI_interface(),
        ...
        self.pop_update = np.zeros(len(ref_dirs))
    ...
    def _next(self):
        ...
        self.pop_update = np.zeros(len(pop))
        ...
        for k in np.random.permutation(len(pop)):
            ...
            # N = self.neighbors[k]
            # updata_id = self.pop_update[self.neighbors[k]]==1
...
parse_doc_string(MOEAD_LLM.__init__)
