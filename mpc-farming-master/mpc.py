...
# Import LED simulation functions from your updated code
...
class LEDPlant:
    """LED plant model for MPC using imported LED functions"""
    ...
    def step(self, pwm_percent, dt=0.1):
        ...
        # Calculate photosynthesis rate if model available
    ...
    def get_photosynthesis_rate(self, ppfd, temperature):
        """Get photosynthesis rate using predictor or simple model"""
...
class LEDMPCController:
    ...
    def __init__(self, plant, prediction_horizon=10, control_horizon=5, dt=0.1):
        ...
        # Constraints - Updated ambient temperature range to (20,30)°C
        self.pwm_min = 0.0
        self.pwm_max = 100.0
        self.dpwm_max = 20.0
    ...
    def set_weights(
        self, Q_ppfd=0.5, Q_temp=0.5, Q_photo=5.0, R_pwm=0.01, R_dpwm=0.5, R_power=0.001
    ):
        """Set MPC weights"""
        self.Q_ppfd = Q_ppfd
        self.Q_temp = Q_temp
...
class LEDMPCSimulation:
    ...
    def create_reference_trajectory(self, duration, dt):
        ...
        if self.pn is not None and self.pn.is_loaded:
            photo_target = self.pn.predict(ppfd_target, temp_target)
            if photo_target is None:
                # Fallback to simple model if prediction fails
                pn_max = 25
                km = 300
    ...
    def run_simulation(self, duration=120, dt=1.0):
        ...
        for k in range(steps):
            ...
            ppfd_ref = ppfd_ref_full[k:k_end] if k < len(ppfd_ref_full) else ppfd_ref_full[-1:]
            temp_ref = temp_ref_full[k:k_end] if k < len(temp_ref_full) else temp_ref_full[-1:]
            photo_ref = photo_ref_full[k:k_end] if k < len(photo_ref_full) else photo_ref_full[-1:]
            ...
            # 安全地添加参考数据
            if k < len(ppfd_ref_full):
                self.ppfd_ref_data.append(ppfd_ref_full[k])
            else:
                self.ppfd_ref_data.append(ppfd_ref_full[-1])
                
            if k < len(temp_ref_full):
                self.temp_ref_data.append(temp_ref_full[k])
            else:
                self.temp_ref_data.append(temp_ref_full[-1])
                
            if k < len(photo_ref_full):
                self.photo_ref_data.append(photo_ref_full[k])
            else:
                self.photo_ref_data.append(photo_ref_full[-1])
...
if __name__ == "__main__":
    ...
    controller.set_weights(
        Q_ppfd=0.3, Q_temp=0.3, Q_photo=10.0, R_pwm=0.01, R_dpwm=0.1, R_power=0.001
    )
    ...
    controller.set_constraints(
        ...
        dpwm_max=10.0,
        temp_min=20.0,
        temp_max=29.0,  # Reduced max PWM and temp
    )
