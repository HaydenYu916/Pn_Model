...
class LEDMPPIController:
    ...
    def __init__(self, plant, horizon=10, num_samples=1000, dt=0.1, temperature=1.0):
        ...
        self.R_power = 0.01  # Power consumption penalty
    ...
    def compute_cost(self, pwm_sequence, current_temp):
        """Compute cost for a single PWM sequence - maximize photosynthesis"""
        try:
    ...
    def solve(self, current_temp, mean_sequence=None):
        """Solve MPPI optimization for photosynthesis maximization"""
...
if __name__ == "__main__":
    ...
    controller.set_weights(
        Q_photo=10.0,  # High weight for photosynthesis maximization
        R_pwm=0.001,  # Low control penalty
        R_dpwm=0.05,  # Smooth control
        R_power=0.01,  # Power consumption penalty
    )
    ...
    results = simulation.run_simulation(duration=30, dt=1.0)
